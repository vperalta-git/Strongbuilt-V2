...
function verifyApprovedLocalImages() {
  assert.equal(Object.keys(approvedLocalTruckImagePaths).length, 54)
  for (const [slug, publicPath] of Object.entries(approvedLocalTruckImagePaths)) {
    assert.match(slug, /^(isuzu|asiastar)-/)
    assert.match(publicPath, /^\/images\/trucks\/(isuzu|asiastar)\/[a-z0-9-]+\.webp$/)
