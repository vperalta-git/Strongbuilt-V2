  ...
  return (
    <footer className="bg-ink text-white">
        ...
        <Container className="grid lg:grid-cols-[1.15fr_0.85fr]">
          <div className="border-white/10 py-14 lg:border-r lg:py-20 lg:pr-14">
            ...
            <div className="mt-8 flex flex-wrap gap-3">
              <Link className="border border-white/50 px-4 py-3 text-xs font-bold uppercase tracking-[0.14em] text-white transition-colors hover:border-brand hover:text-brand" href="/request-a-quote">
                Request a quote
              </Link>
              <Link className="border border-white/50 px-4 py-3 text-xs font-bold uppercase tracking-[0.14em] text-white transition-colors hover:border-brand hover:text-brand" href="/trucks">
                Browse trucks
              </Link>
            </div>
          </div>
          ...
          <div className="grid gap-10 py-14 sm:grid-cols-2 lg:py-20 lg:pl-14">
            <div>
              ...
              <ul className="space-y-4 text-sm leading-6 text-white/75">
                <li>
      ...
      <Container className="grid gap-8 py-10 md:grid-cols-[1fr_auto] md:items-end">
        <div className="flex flex-wrap gap-x-6 gap-y-3 text-xs font-bold uppercase tracking-[0.12em] text-white/68">
          {siteConfig.nav.map((item) => <Link key={item.href} href={item.href} className="hover:text-white">{item.label}</Link>)}
          <Link href="/privacy" className="hover:text-white">Privacy</Link>
        </div>
        <p className="text-xs leading-5 text-white/68 md:text-right">© {new Date().getFullYear()} {siteConfig.legalName}</p>
      </Container>
    </footer>
  )
}
